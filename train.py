...
def train():
    ...
    for epoch in range(
        ...
        if prebias:
            if epoch < 3:  # prebias
                ps = (
                    np.interp(epoch, [0, 3], [0.1, hyp["lr0"]]),
                    0.0,
                )  # prebias settings (lr=0.1, momentum=0.0)
            ...
            if (
                optimizer.param_groups[2].get("momentum") is not None
            ):  # for SGD but not Adam
        ...
        for i, (
            ...
            if opt.multi_scale:
                ...
                if sf != 1:
                    ns = [
                        math.ceil(x * sf / 32.0) * 32 for x in imgs.shape[2:]
        ...
        if not opt.notest or final_epoch:  # Calculate mAP
            ...
            results, maps = test.test(
                ...
                model=model,
                conf_thres=1e-3
                if opt.evolve or (final_epoch and is_coco)
                else 0.1,  # 0.1 faster
                iou_thres=0.6,
    ...
    if len(n):
        ...
        os.rename(wdir + "last.pt", wdir + flast) if os.path.exists(
            wdir + "last.pt"
        ) else None
        os.rename(wdir + "best.pt", wdir + fbest) if os.path.exists(
            wdir + "best.pt"
        ) else None
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--out", type=str, default="data", help="out path for weights and images"
    )
    parser.add_argument(
        "--hyp",
        type=str,
        default="hyp.txt",
        help="custom hyperparameter path",
        required=False,
    )
