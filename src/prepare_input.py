...
def clearImage(frame):
    ...
    for idx in range(len(channels)):
        channels[idx] = cv.max(
        				cv.add(cv.subtract(channels[idx].astype(np.float32), int(a_max_dst[idx]))/image_t,
...
def main():
    ...
    parser.add_argument("--cols", "-c", "--cols", help="model input columns", type=int, default=416)
    parser.add_argument("--rows", "-r", help="model input rows", type=int, default=416)
    args = parser.parse_args()
	
	# Run tests
    gpu_time_0, n_frames = ProcVid(partial(ProcFrameCuda,size=(args.cols, args.rows)), args.vid_path)
