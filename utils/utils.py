...
class FocalLoss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        loss *= (
            self.alpha * (1.000001 - torch.exp(-loss)) ** self.gamma
...
def compute_loss(p, targets, model, giou_flag=True):  # predictions, targets, model
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if nb:  # number of targets
            ...
            tobj[b, a, gj, gi] = (
                giou.detach().clamp(0).type(tobj.dtype) if giou_flag else 1.0
            )
...
def build_targets(model, targets):
    ...
    for i in model.yolo_layers:
        ...
        if multi_gpu:
            ng, anchor_vec = (
                model.module.module_list[i].ng,
                model.module.module_list[i].anchor_vec,
            )
        else:
...
def non_max_suppression(
    ...
    for image_i, pred in enumerate(prediction):
        ...
        if method == "vision_batch":
            ...
            output[image_i] = pred[
                torchvision.ops.boxes.batched_nms(pred[:, :4], pred[:, 4], c, iou_thres)
            ]
            continue
        ...
        for c in cls.unique():
            ...
            if method == "vision":
                det_max.append(
                    dc[torchvision.ops.boxes.nms(dc[:, :4], dc[:, 4], iou_thres)]
                )
